...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # print(noisyDistance)
        # print(sum(emissionModel.values()))
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions:
            for pos, prob in self.getPositionDistribution(self.setGhostPosition(gameState, p)).items():
                #print pos, prob
                allPossible[pos] += self.beliefs[p] * prob
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            ...
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] += self.getBeliefDistribution()[p] * emissionModel[trueDistance]
    ...
    def elapseTime(self, gameState):
        ...
        #     new_pos = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
        #     new_particles.extend([util.sample(new_pos)])
        # self.particles = new_particles
        allPossible = util.Counter()
        for pos in self.particles:
            allPossible[pos] = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
        nextPCs = []
        ...
        for part in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is not None:
                    p *= emissionModels[i][util.manhattanDistance(pacmanPosition, part[i])]
                else:
